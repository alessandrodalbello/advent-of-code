...
9. [Encoding Error](#day-9-encoding-error)
10. [Adapter Array](#day-10-adapter-array)
11. [Seating System](#day-11-seating-system)
12. [Rain Risk](#day-12-rain-risk)
13. [Shuttle Search](#day-13-shuttle-search)
14. [Docking Data](#day-14-docking-data)
15. [Rambunctious Recitation](#day-15-rambunctious-recitation)
16. [Ticket Translation](#day-16-ticket-translation)
17. [Conway Cubes](#day-17-conway-cubes)
18. [Operation Order](#day-18-operation-order)
19. [Monster Messages](#day-19-monster-messages)
20. [Jurassic Jigsaw](#day-20-jurassic-jigsaw)
21. [Allergen Assessment](#day-21-allergen-assessment)
22. [Crab Combat](#day-22-crab-combat)
23. [Crab Cups](#day-23-crab-cups)
24. [Lobby Layout](#day-24-lobby-layout)
25. [Combo Breaker](#day-25-combo-breaker)
...
contain at least 1 time and at most 3 times.
...
In the above example, these slopes would find 2, 7, 3, 4, and 2 trees respectively; multiplied together, these produce
the answer 336.
...
the bags, even if the nesting becomes topologically impractical!
...
576
```
In this list, adding up all the numbers from 15 through 40 produces the invalid number from step 1, 127. (Of course, the
...
not valid for any field. Adding together all the invalid values produces your ticket scanning error rate:
4 + 55 + 12 = 71.
...
For example, given the cube at x=1,y=2,z=3, its neighbors include the cube at x=2,y=2,z=2, the cube at x=0,y=2,z=3, and
so on.
...
For example, given the cube at x=1,y=2,z=3,w=4, its neighbors include the cube at x=2,y=2,z=3,w=3, the cube at
...
letters and one pair with different letters. This leaves eight possibilities: `aaab`, `aaba`, `bbab`, `bbba`, `abaa`,
`abbb`, `baaa`, or `babb`.
...
of rule 0; there can't be extra unmatched characters in the message. (For example, `aaaabbb` might appear to match rule
...
of space cards! You'd like to play a game of Combat, and there's even an opponent available: a small crab that climbed
...
10
```
This arrangement means that player 1's deck contains 5 cards, with 9 on top and 1 on the bottom; player 2's deck also
...
you challenge the small crab to a game of Recursive Combat.

Recursive Combat still starts by splitting the cards into two decks (you offer to play with the same starting decks as
before - it's only fair). Then, the game consists of a series of rounds with a few changes:
...
above the other card). Note that the winner's card might be the lower-valued of the two cards if they won the round due
to winning a sub-game. If collecting cards by winning the round causes a player to have all of the cards, they win, and
the game ends.
...
example, if player 1 draws the 3 card, and player 2 draws the 7 card, this would occur if player 1 has at least 3 cards
left and player 2 has at least 7 cards left, not counting the 3 and 7 cards that were drawn.)

To play a sub-game of Recursive Combat, each player creates a new deck by making a copy of the next cards in their deck
(the quantity of cards copied is equal to the number on the card they drew to trigger the sub-game). During this
sub-game, the game that triggered it is on hold and completely unaffected; no cards are removed from players' decks to
form the sub-game. (For example, if player 1 drew the 3 card, their deck in the sub-game would be copies of the next
...
rules as regular Combat. In the above game, the winning player's score is 291.
...
identified by a series of these directions with no delimiters; for example, `esenee` identifies the tile you land on if
